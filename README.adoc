...
Antq runs on https://babashka.org[babashka] 1.13.224 or later, which is
where `clojure.data.json` first ships. Until that release, use a
https://github.com/babashka/babashka-dev-builds[dev build].

[source,sh]
----
bb -Sdeps '{:deps {com.github.liquidz/antq {:mvn/version "RELEASE"}}}' \
  -m antq.core
----

One difference from the JVM: SNAPSHOT versions of Maven artifacts are not reported.
