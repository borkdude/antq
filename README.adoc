...
Antq runs on https://babashka.org[babashka] 1.13.223 or later.
...
Two differences from the JVM:

* SNAPSHOT versions of Maven artifacts are not reported.
* Changelog and diff URLs are found only for artifacts in repositories that serve their POM without authentication.
