...
== Unreleased (dev)
=== Added
* Run on babashka 1.13.224 or later, the first release with `clojure.data.json`. SNAPSHOT versions of Maven artifacts are not reported.

=== Changed
* Read an artifact's POM from the local Maven repository through tools.deps instead of resolving its repository and fetching the POM over HTTP. Artifacts whose repository needs authentication now get a changelog URL too.
